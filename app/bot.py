...
class TenantBot:
    def __init__(self, user_id, mode="user", api_id=None, api_hash=None, session_string=None, bot_token=None):
        ...
        self._away_sent = {}      # customer_id -> ts (business-hours away cooldown)
        self._media_cooldown = {} # (customer_id, media_name) -> ts
    ...
    async def _handle(self, event):
        ...
        try:
            await db.touch_customer(self.user_id, customer_id, via="user")
        ...
        # ── Business hours (optional): reply only within active hours ──
        if persona.get("hours_enabled"):
            ...
            if not within:
                ...
                if away and time.time() - self._away_sent.get(customer_id, 0) > 7200:
                    ...
                    try:
                        await self.client.send_message(customer_id, away)
                    except Exception:
                        pass
                return
    ...
    async def run_followups(self):
        ...
        try:
            ...
            for c in customers:
                if (c.get("via") or "user") != "user":
                    continue  # personal-account bot only follows up personal-account customers
